...
def detect_emotion(polarity, subjectivity, text):
    ...
    if any(w in text.lower() for w in
           ["sad", "cry", "depressed", "lonely", "miss"]):
        traits.append("😢 Sadness Detected")

    if any(w in text.lower() for w in
           ["scared", "afraid", "fear", "nervous", "anxious"]):
...
def make_chart(polarity, subjectivity):
    ...
    # Polarity Chart
    pol_color = (
        '#4CAF50' if polarity >= 0.1
        else '#f44336' if polarity <= -0.1
        else '#FF9800'
    )

    axes[0].barh(['Polarity'], [polarity],
                 color=pol_color, height=0.4)
...
def analyze_single(text, language):
    ...
    emotion, emotion_desc, traits = detect_emotion(
        polarity,
        subjectivity,
        translated
    )
    ...
    traits_str = (
        "\n".join(traits)
        if traits
        else "No special traits detected."
    )
    ...
    polarity_str = (
        f"{polarity:.2f}  "
        f"(Scale: -1.0 Negative ← 0 → +1.0 Positive)"
    )

    subjectivity_str = (
        f"{subjectivity:.2f}  "
        f"(0.0 = Fully Objective, 1.0 = Fully Subjective)"
    )
...
def analyze_csv(file, language):
    ...
    # Summary
    summary = (
        f"✅ Analyzed {len(results)} reviews successfully!\n\n"
    )
...
with gr.Blocks(
    ...
    gr.Markdown(
        "# 🧠 AI Emotion & Sentiment Analysis System"
    )

    gr.Markdown(
        "Analyze emotions in single text or bulk CSV files "
    ...
    with gr.Tabs():
        ...
        with gr.Tab("📝 Single Text Analysis"):
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    polarity_out = gr.Textbox(
                        label="📊 Polarity Score"
                    )

                    subjectivity_out = gr.Textbox(
                        label="🧠 Subjectivity Score"
                    )

                    traits_out = gr.Textbox(
        ...
        with gr.Tab("📁 Bulk CSV Analysis"):
            ...
            gr.Markdown(
                ...
                "`review`, `text`, `comment`, or `feedback`"
            )
            ...
            csv_chart = gr.Plot(
                label="📊 Emotion Distribution Chart"
            )
